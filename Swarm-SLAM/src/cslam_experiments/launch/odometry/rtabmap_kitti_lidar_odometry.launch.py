    ...
    return [
        ...
        Node(
            ...
            arguments=[LaunchConfiguration("args"), LaunchConfiguration("odom_args"),
                # Keep the comma: adjacent string literals concatenate in Python,
                # and '--ros-args' '--log-level' silently became the single
                # unrecognized token '--ros-args--log-level', so log_level was
                # never applied and this node always ran at the default INFO.
                '--ros-args',
                '--log-level',
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # tf2_ros always publishes to the absolute "/tf"/"/tf_static" topics regardless of
        # node namespace, so multiple namespaced robots sharing unnamespaced sensor frame_ids
        # (e.g. this launch file's own "base_link" default) would collide on one global TF
        # tree. Override these to a per-namespace topic (e.g. "/r0/tf") for true multi-robot
        # isolation; left as "/tf"/"/tf_static" by default so single-robot/other callers are
        # unaffected.
        DeclareLaunchArgument('tf_topic',        default_value='/tf',        description='Topic to remap this node\'s "/tf" broadcasts/lookups to - set per-robot (e.g. "/r0/tf") to isolate TF trees in multi-robot setups.'),
        DeclareLaunchArgument('tf_static_topic', default_value='/tf_static', description='Same as tf_topic, for "/tf_static".'),
