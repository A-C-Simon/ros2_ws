...
import rclpy
# solve_teaser() logs via rclpy.logging on its failure path. Importing the
# parent package alone does not bind the submodule, so that call only worked
# when something else in the process happened to have imported it - a plain
# "import rclpy" raises AttributeError instead of logging the failure.
...
def points_to_open3d(points):
    """Wrap an (N, 3) array of points in an Open3D cloud.

    Callers that retain many keyframes should keep the plain array and build a
    cloud only when one is actually needed. Besides being far smaller (see
    downsample_ros_pointcloud_to_array), a freshly built cloud protects the
    retained copy from extract_fpfh(), which calls estimate_normals() *in
    place* and would otherwise attach a float64 normals array - as large again
    as the points - to every stored keyframe that a loop closure is attempted
    against.
    """
    open3d_cloud = open3d.geometry.PointCloud()
...
def points_to_ros(points):
    header = Header()
    fields = FIELDS_XYZ
    return pc2_utils.create_cloud(
        header, fields, np.asarray(points, dtype=np.float32).reshape(-1, 3))


def open3d_to_ros(open3d_cloud):
    return points_to_ros(np.asarray(open3d_cloud.points))
...
def downsample_ros_pointcloud_to_array(pc_msg, voxel_size):
    """Same as downsample_ros_pointcloud, but returns a compact float32 array.

    Open3D stores points as float64 (Vector3dVector) inside a PointCloud
    object. For a KITTI keyframe at voxel_size 0.5 that is ~345 kB of point
    data per keyframe, against ~172 kB as float32 - and the incoming ROS
    PointField is FLOAT32 to begin with, so the extra precision is carrying no
    information (measured round-trip error 3.8e-06 m, against a 0.5 m voxel and
    a 4.0 m ICP correspondence distance).
    """
    return np.asarray(downsample_ros_pointcloud(pc_msg, voxel_size).points,
                      dtype=np.float32)
